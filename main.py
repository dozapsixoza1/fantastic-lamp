...
# Конфигурация
API_ID = '26840045'
API_HASH = '76ef01e235c41042d8aedb58ff1fff51'
BOT_TOKEN ='8525188022:AAGmgw2Yqbn1UDt1Eqxgbw_q7nrewKuySrA'
ADMINS = [7504103313]
LOG_CHANNEL = 'https://t.me/+Fsf6BknYmO9iYTMy'  # Ссылка на канал логов
...
main_buttons = [
    ...
    [Button.text("✅ Гаранты Чата", resize=True), Button.text("👨‍🎓 Админы Чата", resize=True)],
    [Button.text("🔰 Проверенные пользователи", resize=True), Button.text("📊 Статистика базы", resize=True)],
    [Button.text("🔓 Премиум", resize=True), Button.text("❓ Частые вопросы", resize=True)],
    [Button.text("🚫 Слить скаммера!", resize=True)],
    [Button.text("🔗 Проверить ссылку", resize=True)]  # Новая кнопка
]

# Роли пользователей
ROLES = {
    0: {"name": "Нет в базе 📝", "preview_url": "https://i.ibb.co/zTJzG4wy/684-20250918160454.jpg", "scam_chance": 31},
    1: {"name": "Гарант 🛡️", "preview_url": "https://i.ibb.co/2YMpLn7Z/684-20250918165136.jpg", "scam_chance": 1},
    2: {"name": "Возможно скамер ⚠️", "preview_url": "https://i.ibb.co/qM8cgT4W/684-20250919142142.jpg", "scam_chance": 65},
    3: {"name": "Скамер ❌", "preview_url": "https://i.ibb.co/8LRCsmYR/684-20250919140913.jpg", "scam_chance": 99},
    4: {"name": "Петух 🐓", "preview_url": "https://i.ibb.co/MkPXyd5r/684-20250919141532.jpg", "scam_chance": 45},
    5: {"name": "Подозрение на скам ⚠️", "preview_url": "https://i.ibb.co/qM8cgT4W/684-20250919142142.jpg", "scam_chance": 51},
    6: {"name": "Стажёр 🎓", "preview_url": "https://i.ibb.co/7xQxGjPR/684-20250918164710.jpg", "scam_chance": 20},
    7: {"name": "Админ 👮", "preview_url": "https://i.ibb.co/Zp2509QT/684-20250918200530.jpg", "scam_chance": 15},
    8: {"name": "Директор 👔", "preview_url": "https://i.ibb.co/C5DzMKNy/684-20250919135629.jpg", "scam_chance": 10},
    9: {"name": "Президент 👑", "preview_url": "https://i.ibb.co/rRN6RmqR/684-20250919135959.jpg", "scam_chance": 5},
    10: {"name": "Создатель ⭐", "preview_url": "https://i.ibb.co/GQpxzVgW/684-20250918163601.jpg", "scam_chance": 1},
    11: {"name": "Кодер 💻", "preview_url": "https://i.ibb.co/0xRK7dd/image.png", "scam_chance": 3},
    12: {"name": "Проверен гарантом ✅", "preview_url": "https://i.ibb.co/XhnMmCT/684-20250918171704.jpg", "scam_chance": 5},
    13: {"name": "Липофрения", "preview_url": "https://i.ibb.co/0xRK7dd/image.png", "scam_chance": 0}
}
...
def get_db_connection():
    try:
        conn = sqlite3.connect('scamers.db')  # Замените на имя вашей базы данных
        return conn
...
class Database:
    def __init__(self, db_name='code.db'):
    ...
    def create_tables(self):
        logging.info("Создание таблиц если они не существуют...")
        ...
        # Добавляем начальную запись если таблица пустая
    ...
    def update_role(self, user_id, role_id, granted_by_id=None):
        try:
            ...
            # Если granted_by_id предоставлен, обновляем его
...
def get_guarantors():
    ...
    cursor = conn.cursor()
    cursor.execute('SELECT user_id FROM users WHERE role_id = 1')  # Предположим, что role_id = 1 - это гарант
...
def get_trainees():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM trainees")  # Замените на ваш SQL-запрос
    trainees = cursor.fetchall()
    conn.close()
    return trainees

# Инициализация бота
bot = TelegramClient('soul.session', API_ID, API_HASH).start(bot_token=BOT_TOKEN)
db = Database()
...
async def get_user_profile_response(event, user, user_data):
    ...
    custom_image_url = db.get_user_custom_photo(user_id)
    ...
    if role_id == 0:
        message_text = (
            ...
            f"[❌] Статус: не найден в базе. Риск скама: **38%**\n"
            ...
            f"[🔒] Используйте Гарантов Moncler для безопасных сделок.\n\n"
        ...
        message_text = (
            f"[👤]({custom_image_url})[ {user.first_name} ](tg://user?id={user_id}) #id{user_id} [⠀]({ROLES[role_id]['preview_url']})\n\n"
            f"[✅] Статус: Гарант\n"
            f"[ℹ️] [Узнайте о гарантах](https://telegra.ph/Kto-takoj-GARANT-05-29)\n\n"
            f"[📍] Регион: {country_display}\n"
            f"[🚫] Разоблачено скаммеров: {scammers_slept}\n\n"
            f"[🔒] Данный пользователь является проверенным гарантом Moncler\n\n"
            f"[📅] Дата: {current_time} | 🔎Проверок: {checks_count}\n"
        )
        ...
        message_text = (
            ...
            f"[📍] Регион: {country_display}\n"
            f"[🚫] Разоблачено скаммеров: {scammers_slept}\n\n"
            f"[🔒] Данный пользователь является Cоздателем базы Moncler\n\n"
            f"[📅] Дата: {current_time} | 🔎Проверок: {checks_count}\n"
        )
        ...
        message_text = (
            ...
            f"[🔒] Данный пользователь является надёжным президентом чата Moncler\n\n"
            f"[📅] Дата: {current_time} | 🔎Проверок: {checks_count}\n"
        )
        ...
        message_text = (
            ...
            f"[🚫] Разоблачено скаммеров: {scammers_slept}\n\n"
            f"[⚠] Выговоры: {warnings_count} "
            f"[🔒] Данный пользователь является Администратором чата Moncler\n\n"
            f"[📅] Дата: {current_time} | 🔎Проверок: {checks_count}\n"
        )
        ...
        message_text = (
            ...
            f"[⚠] Выговоры: {warnings_count}\n"
            f"[📣] Канал: {channel}\n\n"         
            f"[🔒] Данный пользователь является Стажёром Чата Moncler\n\n"
            f"[📅] Дата: {current_time} | 🔎Проверок: {checks_count}\n"
        )
        ...
        message_text = (
            ...
            f"[⚠] Выговоры: {warnings_count}\n"
            f"[📣] Канал: {channel}\n\n"         
            f"[🔒] Данный пользователь является Директором Чата Moncler\n\n"
            f"[📅] Дата: {current_time} | 🔎Проверок: {checks_count}\n"
        )
        ...
        message_text = (
            ...
            f"[⚠] Выговоры: {warnings_count}\n"
            f"[📣] Канал: {channel}\n\n"         
            f"[🔒] Данный пользователь является Техническим Специалистом Чата Moncler\n\n"
            f"[📅] Дата: {current_time} | 🔎Проверок: {checks_count}\n"
        )
    else:
...
async def check_user(event):
    ...
    if user_id in last_check_time:
        ...
        if elapsed_time < 5:  # Если прошло меньше 5 секунд
            ...
            return await send_response(event, f"💄 Братаны терпели, и ты потерпи {remaining_time:.1f} секунд(ы)!")
...
async def cur_command(event):
    ...
    user_role = db.get_user_role(sender.id)

    if user_role not in [8, 10]:  # 8 - директор, 10 - владелец
        await event.respond("❌ У вас нет прав для выполнения этой команды.")
        return
    ...
    db.cursor.execute('UPDATE users SET curator_id = ? WHERE user_id = ?', (sender.id, target.id))
    db.conn.commit()

    await event.reply(f"✅ Стажёру [{target.first_name}](tg://user/{target.id}) назначен курирующий: [{sender.first_name}](tg://user/{sender.id}).")
...
async def warning_handler(event):
    ...
    if user_role not in [7, 8, 9, 10]:  # Только админ, директор, президент
...
**Ice Премиум:** {'✅' if db.get_premium_expiry(user_id) else '❌'}
[⠀](https://i.ibb.co/QF3TxtWG/temp-5173733679-1333.jpg)
"""
...
async def thank_command(event):
    ...
    allowed_roles = [6, 8, 10, 11, 9]  # Стажёр=6, Директор=8, Создатель=10, Кодер=1, Президент=9
    ...
    try:
        ...
        await event.respond(
            f"📛 Хорошо!, вы успешно выдали +1 слитого скаммера пользователю с ID {target_user_id}.\n\n"
            "📈 Спасибо, что боретесь со скамом вместе с Ice.\n\n"
...
async def start_game(event):
    ...
    db.cur.execute('INSERT INTO games (player1_id, player2_id, bet_amount, turn, player1_aimed, player2_aimed) VALUES (?, ?, ?, ?, ?, ?)', 
...
async def shoot(event):
    ...
    game = db.cur.execute('SELECT * FROM games WHERE player1_id = ? OR player2_id = ?', (event.sender_id, opponent_id)).fetchone()
    if not game:
        await event.answer("❌ Игра не найдена!", alert=True)
        return
    ...
    if random.randint(1, 100) <= chance:
        ...
        await event.respond("✅ Вы успешно уничтожили корабль противника!", file="https://i.ibb.co/DfSQZk0Z/temp-5173733679.jpg")
    else:
        # Провал
...
async def update_turn(event, opponent_id):
    ...
    db.cur.execute('UPDATE games SET turn = ? WHERE player1_id = ? OR player2_id = ?', (1 if event.sender_id == opponent_id else 2, event.sender_id, opponent_id))
    db.conn.commit()
...
async def start(event):
    ...
    inline_buttons = [
        [Button.inline("🖌️ | Кастом картинки", "custom_pics"),
         Button.inline("👋 | Добавить в группу", "add_group")],
        [Button.inline("🔥 | Пожаловаться на скамера", "report_scammer")],
        [Button.inline("🤗 | Создатель", "creator")],
        [Button.inline("💝 Поддержать", "support_handler")],  # Новая кнопка поддержки
        [Button.inline("ℹ️ О проекте", "about_project")]  # Новая кнопка о проекте
    ]

    welcome_text = (
        "Мы - Ice Глобальная АнтиСкам база нового поколения[⠀](https://i.ibb.co/67gnBzKm/temp-5173733679-1351.jpg)!\n"
        f"🤓 Проверим человека на скам\n"
    ...
    await event.respond(
        """🍀 | 🎮 Меню бота, используйте кнопки ниже для навигации.

🌿 | Для проверки Пропишите:
<pre>чек @dixtirmorg</pre>""",
...
async def about_project(event):
    about_text = (
        "Мы глобальная анти скам база Ice. Наш персонал помогает комьюнити бороться со скамом. "
        "Если у вас есть ещё какие-то вопросы, пишите, с радостью вам помогу🤗[⠀](https://i.ibb.co/S40fy8Kx/temp-5173733679-1327.jpg)"
    )
...
async def how_to_buy_trust(event):
    response_text = (
        ...
        "Гарантов ты можешь найти в чате или же нажав на кнопку 'Гаранты' в главном меню🤑[⠀](https://i.ibb.co/DfhjP6QT/2.jpg)"
    )
    await event.respond(response_text)
...
async def how_to_report_scammer(event):
    response_text = (
        ...
        "Наши волонтёры занесут этого скаммера😈[⠀](https://i.ibb.co/Z11xspsh/temp-5173733679-1329.jpg)"
    )
    await event.respond(response_text)
...
async def support_handler(event):
    support_text = (
        ...
        "Если вы хотите поддержать кодера, дайте ему звезд ник: **@dixtirmorg**[⠀](https://i.ibb.co/0x7KTr0/image.jpg)"
    )

    buttons = [
        [Button.url("💌 Крипто ботом", "http://t.me/send?start=IVZxFAIV2eDw")],
        [Button.url("💞 Наш кодер", "https://t.me/dixtirmorg")]
    ]
    ...
    async def site_link_handler(message):
        try:
            ...
            if re.match(r"https?://[^\s]+", link) and len(link) < 100:
                await message.reply("Ссылка безопасна! Но учтите, в ссылке может быть логгер. Рекомендуем проверить ссылку, нажав на 3 кнопку.")
            else:
    ...
    async def logger_check_handler(message):
        try:
            ...
            if re.search(r"(virus|malware|stealer)", link, re.IGNORECASE):
                await message.reply("⚠️ Внимание! Ссылка может содержать вредоносный код (стиллер/логер). Не переходите по ней!")
            else:
                await message.reply("✅ Ссылка выглядит безопасной, но всегда будьте осторожны при переходе по неизвестным ссылкам.")
        except Exception as e:
...
async def show_statistics(event):
    ...
    statistics = (
        f"📊 **Статистика чата:[⠀](https://i.ibb.co/yF4H4pQ0/temp-5173733679-1181.jpg)**\n"
...
async def handle_role_command(event):
    user_role = db.get_user_role(event.sender_id)
    is_admin = event.sender_id == 8456889428  or event.sender_id == 8456889428 or user_role == 10
    ...
    if action == '+':
        ...
        # Специальные права для ID 5288146757  и 6974880433⠀
        if event.sender_id in [8456889428] and role in ['кодер', 'создатель']:
        ...
        # Снятие ролей
        if current_role == 10 and event.sender_id in [8456889428]:
...
async def mute_handler(event):
    ...
    try:
        ...
        keyboard = [
            ...
            [Button.url("👓 Чат для оффтопа", "https://t.me/offtopIce")],
...
async def unmute_handler(event):
    ...
    try:
        ...
        keyboard = [
            [Button.url("👑 Чат для оффтопа", "https://t.me/+JQUcQmJ_gcYzYzUy")],
            [Button.url("📋 Логи", LOG_CHANNEL)]
        ]
...
if len(args) < 4:
    await event.respond("❌ Используйте: /скам @username/ID <ссылка> <причина>")
    return

target = args[1]            # @юзер или ID
link = args[2]              # ссылка
cause = args[3]             # вся причина после ссылки
description = f"{cause} {link}"  # объединяем

logging.info(f"Пользователь {user_id} пытается добавить скамера: {target}, причина: {description}")
...
try:
    ...
    db.add_user(user.id, user.username)  # Добавляем пользователя в базу
    db.add_scammer(user.id, description, user_id, description, unique_id) # Передаем уникальный идентификатор и текстовое описание
...
async def mark_user_handler(event):
    ...
    scammers_slept = current_count + 1  # Увеличиваем на 1
...
# Обработчик для обжалования
@bot.on(events.CallbackQuery(pattern=r'appealll_(\d+)'))
async def appeal_handler(event):
    user_id = int(event.pattern_match.group(1))

    await event.answer("Проверьте личные сообщения!", alert=True)

    # Отправляем сообщение в личные сообщения
    await event.client.send_message(event.sender_id,
                                    "Если вы хотите обжаловать решение Администрации на счёт заноса вас в базу, вот что вам нужно будет сделать!\n"
                                    "1️⃣ Перейти в нашу предложку Grand!\n"
                                    "2️⃣ Описать свою проблему и предоставить доказательства того, что вы не обманывали пользователя!\n"
                                    "3️⃣ Ожидать ответа от модераторов!\n\n"
                                    "Спасибо, что выбрали базу Grand, мы очень ценим наших пользователей!")

@bot.on(events.NewMessage(pattern=r'(?i)^(/unscam|анскам)'))
async def unscam_handler(event):
    """Обработчик снятия статуса скамера"""
    # Получаем роль отправителя
    user_role = db.get_user_role(event.sender_id)
    ...
    if event.is_reply:
        ...
        args = event.raw_text.split()
        if len(args) < 2:
            await event.reply("❌ Используй: /unscam @username или ответом на сообщение")
            return
        try:
            target = await event.client.get_entity(args[1])
        except Exception as e:
            await event.reply("❌ Не могу найти чела")
            return

    # Проверяем текущий статус
    current_role = db.get_user_role(target.id)
    if current_role not in [2, 3, 4, 5]:  # Если не скамер/подозреваемый
        await event.reply("ℹ️ У человека нет статуса")
        return

    # Снимаем статус (устанавливаем стандартную роль)
    db.update_role(target.id, 0)

    # Удаляем из таблицы scammers
    db.cursor.execute('DELETE FROM scammers WHERE scammer_id = ?', (target.id,))
    db.conn.commit()

    await event.reply(
        f"✅ Статус был успешно снят с пользователя [{target.first_name}](tg://user/{target.id})",
        parse_mode='md'
    )
...
async def add_reason_handler(event):
    ...
    logging.info(f"Попытка обновления описания для пользователя {target.first_name} ({target.id}) с новым описанием: {new_description}")
    ...
    try:
        ...
        logging.info(f"Описание для пользователя {target.first_name} ({target.id}) успешно обновлено на: {new_description}")
        await event.reply(f"✅ Описание для пользователя [{target.first_name}](tg://user/{target.id}) обновлено: {new_description}", parse_mode='md')
    except Exception as e:
...
async def add_additional_reason_handler(event):
    ...
    await event.reply(f"✅ Дополнительное описание для пользователя [{target.first_name}](tg://user/{target.id}) добавлено: {additional_reason_text}", parse_mode='md')
...
async def trust_command(event):
    ...
    if db.get_user_role(sender.id) not in [1, 10]:
        await event.reply(
            ...
            "[⠀](https://i.ibb.co/QjhmR9Cp/image.jpg)",
            parse_mode='md',
            link_preview=True
        )
        return
    ...
    try:
        ...
        try:
            await bot.send_message(
                ...
                buttons=Button.url("📢 Предложка", "https://t.me/tokyorevenge_reports")
            )
        except:
            pass
...
buttons = Button.url("📢 Предложка", "https://t.me/ares_report")
                )
            except:
                pass
...
async def statistics(event):
    ...
    loading_message = await event.respond("🔄 Загрузка статистики...\n▰▱▱▱▱▱▱▱▱▱ 10%")
    ...
    for percent, bar in progress_steps:
        ...
        await loading_message.edit(f"🔄 Загрузка статистики...\n{bar} {percent}%")
...
async def top_day_handler(event):
    ...
    try:
        ...
        for i, (user_id, username, count) in enumerate(top_users, 1):
            ...
            response += f"{i}. {username} — ✉️ {count} сообщений\n"
...
async def report_scammer(event):
    ...
    keyboard = types.KeyboardButtonUrl(text="🚨 Отправить жалобу", url="https://t.me/tokyorevenge_reports")
    await event.respond(
...
[⠀](https://i.ibb.co/Z11xspsh/temp-5173733679-1329.jpg)""",
        parse_mode='md',
        link_preview=True,
        buttons=keyboard
    )
...
async def list_garants(event):
    ...
    try:
        ...
        await loading_message.edit(f"Ошибка при получении данных: {e} ⛔")
        return

    if not garants:
        await loading_message.edit("На данный момент Гарантов нету ⛔")
        return

    # Формируем текст с актуальным списком
    text = f"""💢 Актуальный список гарантов Tokyo Revenge
━━━━━━━━━━━━━━
• Всего: {len(garants)}
━━━━━━━━━━━━━━
💡 Если хотите стать гарантом, пройдите набор!
[⠀](https://i.ibb.co/XrQN5hzc/temp-5173733679-1331.jpg)"""
...
async def list_volunteers(event):
    ...
    if not volunteers:
        await loading_message.edit("На данный момент Волонтёров нету ⛔")
        return

    # Формируем текст с актуальным списком
    text = f"""🤝 Актуальный список волонтёров Tokyo Revenge
━━━━━━━━━━━━━━
• Всего: {len(volunteers)}
━━━━━━━━━━━━━━
💡 Если вы хотите стать волонтёром базы, просто пройдите набор!
[⠀](https://i.ibb.co/MYm2Qwz/temp-5173733679-1330.jpg)"""
...
async def list_verified_users(event):
    ...
    if not verified_users:
        await loading_message.edit("На данный момент проверенных пользователей нет ⛔")
        return
...
async def premium_info(event):
    ...
    text = (
        ...
        "Все эти фишки входят в Tokyo Revenge Premium"
    )

    buttons = [
        [Button.url("💰 Оплата", "https://t.me/Werovers")],
...
async def my_profile(event):
    if not event.is_private:
        # Удаляем сообщение, если оно не в ЛС бота
        await event.delete()
        return
    ...
    user_id = event.sender_id  # Используем sender_id для получения ID пользователя

    # Получаем данные пользователя из базы
    user_data = db.get_user(user_id)
    if user_data is None:
        await loading_message.edit("❌ Не удалось найти ваши данные в базе.")
        return

    role_id = user_data[2]  # Предположим, что роль находится на позиции 2
    role_info = ROLES[role_id]  # Получаем информацию о роли
    ...
    # Отладка: выводим значение custom_photo
    logging.info(f"Custom photo for user {user_id}: {custom_photo}")
    ...
    logging.info(f"Статус премиума для пользователя {user_id}: {premium_expiry}")
...
📛 **Роль:** {role_info['name']}
🆔 **ID:** {user_id}[ ](https://files.catbox.moe/w4c7xs.png)
👑 *Tokyo Revenge premium:** {premium_status}
🔍 **Проверок:** {checks_count}
"""

    # Удаляем сообщение о загрузке и отправляем профиль пользователя
    await loading_message.delete()
    await event.respond(
        profile_text,
        buttons=[
...
async def handle_offtopic_command(event):
    ...
    if event.is_reply:
        ...
        try:
            ...
            # Создаем кнопку
            keyboard = [
                [Button.url("Перейти", "https://t.me/tokyorevenge_reports")]
            ]
...
[🤗](https://i.ibb.co/67gnBzKm/temp-5173733679-1351.jpg)
"""
        await event.respond(text, buttons=buttons, parse_mode='md', link_preview=True)
...
async def add_group_handler(event):
    url = "https://t.me/lightingreports?startgroup=newgroup&admin=manage_chat+delete_messages+restrict_members+invite_users+restrict_members+change_info+pin_messages+manage_video_chats"
...
async def report_handler(event):
    await event.respond("Для того что бы пожаловатся на скамера вы должны перейти в наш [специальный чат](https://t.me/+MXZUNK51SYsxYzE6)\nВам нужны пруфы и скрины переписок!")
...
# Обработчик кнопки "Проверить себя"
...
async def check_soon_handler(event):
    ...
    buttons = [
        ...
        [
            Button.url("⚠️ Слить скаммера", "https://t.me/tokyorevenge_reports"),
            Button.inline("⚖️ Аппеляция", "appeal")
        ]
    ]

    await event.edit(response, buttons=buttons, parse_mode='md')
